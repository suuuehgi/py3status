...
Configuration parameters:
    ...
    format: display format for this module
        (default '{server} [\?if=state&color=good DND|\?color=bad DND]')
...
Format placeholders:
    ...
    {state} do not disturb state, eg False, True

Color options:
    color_bad: Do Not Disturb is disabled
    color_good: Do Not Disturb is enabled
...
# display ON/OFF
do_not_disturb {
    format = '{server} [\?if=state&color=good  ON|\?color=bad OFF]'
}
# display 1/0
do_not_disturb {
    format = '{server} [\?if=state&color=good 1|\?color=bad 0]'
}

# display DO NOT DISTURB/DISTURB
do_not_disturb {
    format = '[\?if=state&color=good DO NOT DISTURB|\?color=lightgray DISTURB]'
}
```
...
class Py3status:
    ...
    cache_timeout = 10
    format = "{server} [\?if=state&color=good DND|\?color=bad DND]"
    pause = False
    server = None
    state = "last"
    ...
    def do_not_disturb(self):
        ...
        return {
            ...
            "full_text": self.py3.safe_format(
                self.format, {"state": int(self.state), "server": self.server}
            ),
        }
